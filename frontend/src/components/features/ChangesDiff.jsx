  ...
  <div className="space-y-3">
    {semanticChanges.length ? (
      semanticChanges.map((change, index) => (
        <div key={`${change.field}-${index}`} className="rounded-2xl border border-border bg-surface/60 p-4">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex items-center gap-3">
              <code className="font-mono text-sm text-text-secondary">{change.field}</code>
            ...
            <span className={`inline-flex w-fit rounded-full border px-2.5 py-1 text-[11px] uppercase tracking-[0.2em] ${tone[change.type] || tone.modified}`}>
              {change.type}
            </span>
          </div>
