...
export const ProjectDetail = () => {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const [projectResponse, eventsResponse] = await Promise.all([getProject(id), getEvents(id, { limit: 100 })])
  ...
  const pagedEvents = useMemo(() => events.slice((page - 1) * 20, page * 20), [events, page])
  const status = project ? formatStatus(project.status) : null
    ...
    <div className="space-y-6">
      {loading ? (
        ...
        <>
            ...
            <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
              <div>
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <Badge label={status.label} color={status.color} bg="rgba(99,102,241,0.08)" border="rgba(99,102,241,0.22)" pulse={status.pulse} />
                </div>
                <a href={project.githubRepoUrl} target="_blank" rel="noreferrer" className="mt-3 inline-flex items-center gap-2 text-sm text-indigo-300">
            ...
            <div className="glass-card overflow-hidden rounded-3xl">
                      ...
                      pagedEvents.map((event) => (
                        <tr key={event._id} className="border-b border-border/60 last:border-0">
                          ...
                          <td className="px-5 py-4">
                            <code className="font-mono text-xs text-indigo-200">{formatSha(event.commitSha)} </code>
                            <span className="ml-2 text-text-secondary">{event.commitMessage?.slice(0, 50)}</span>
                          </td>
                          <td className="px-5 py-4 text-text-secondary">
                            {event.changedFiles?.[0] || '—'}
                            {event.changedFiles?.length > 1 ? <span className="ml-2 rounded-full bg-white/5 px-2 py-1 text-xs">+{event.changedFiles.length - 1} more</span> : null}
                          </td>
                          <td className="px-5 py-4">{event.riskScore ? <RiskBadge score={event.riskScore} size="sm" /> : '—'}</td>
                          <td className="px-5 py-4">
                            <Badge
                              label={formatStatus(event.status || 'active').label}
                              color={formatStatus(event.status || 'active').color}
                              bg="rgba(99,102,241,0.08)"
                              border="rgba(99,102,241,0.22)"
                              pulse={formatStatus(event.status || 'active').pulse}
                              size="sm"
                            />
                          </td>
                          <td className="px-5 py-4">
              ...
              {events.length > 20 ? (
                <div className="flex items-center justify-center gap-2 border-t border-border p-4">
                  {Array.from({ length: Math.ceil(events.length / 20) }, (_, index) => index + 1).map((item) => (
                    <button
                      key={item}
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div className="rounded-2xl border border-border bg-white/5 p-4">
                  ...
                  <p className="mt-2 text-text-primary">
                    {project.branch} · {project.folderPath}
                  </p>
                </div>
              </div>
          ...
          <ProjectForm project={project} isOpen={isEditing} onClose={() => setIsEditing(false)} onSaved={load} />
        </>
      ) : (
        <div className="glass-card rounded-3xl p-6">
